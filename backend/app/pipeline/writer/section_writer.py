...
12. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО добавлять список литературы, библиографию, перечень источников или блок «Список использованных источников» в конце раздела — библиография генерируется автоматически отдельно
13. Ссылайся на источники СТРОГО по их номерам [N] из РЕЕСТРА ИСТОЧНИКОВ — НЕ придумывай свои номера и НЕ используй номера, которых нет в реестре

ДОПОЛНИТЕЛЬНЫЕ ИНСТРУКЦИИ:
{additional_instructions}

Напиши ТОЛЬКО текст раздела, без заголовка и без списка литературы. Если в конце текста появится что-то похожее на библиографический список — это ОШИБКА."""
...
2. Степень разработанности проблемы (1-2 абзаца, ОБЯЗАТЕЛЬНО используй минимум 5-8 РАЗНЫХ ссылок на авторов из РЕЕСТРА ИСТОЧНИКОВ в формате [N], покажи широту изученности проблемы)
...
11. Ссылайся на источники СТРОГО по их номерам [N] из РЕЕСТРА ИСТОЧНИКОВ — НЕ придумывай свои номера и НЕ используй номера, которых нет в реестре

Объём: примерно {target_words} слов. Академический стиль.
НЕ используй markdown-разметку (**, ##, ` и т.д.) — пиши чистый текст.
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО добавлять список литературы, библиографию, перечень источников или блок «Список использованных источников» в конце — библиография генерируется автоматически отдельно.

Напиши ТОЛЬКО текст введения. Если в конце текста появится что-то похожее на библиографический список — это ОШИБКА."""
...
7. Ссылайся на источники СТРОГО по их номерам [N] из РЕЕСТРА ИСТОЧНИКОВ — НЕ придумывай свои номера и НЕ используй номера, которых нет в реестре
8. Объём: примерно {target_words} слов
9. НЕ используй markdown-разметку (**, ##, ` и т.д.) — пиши чистый текст
10. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО добавлять список литературы, библиографию, перечень источников или блок «Список использованных источников» в конце — библиография генерируется автоматически отдельно

Напиши ТОЛЬКО текст заключения. Если в конце текста появится что-то похожее на библиографический список — это ОШИБКА."""
