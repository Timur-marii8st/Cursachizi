...
class TestSectionWriter:
    ...
    async def test_write_section_includes_section_contract(
        self,
        mock_llm: MockLLMProvider,
        outline: Outline,
        sources: list[Source],
        ...
        brief = SectionBrief(
            chapter_number=1,
            chapter_title=outline.chapters[0].title,
            section_title="1.1 ??????? ????????????",
            chapter_description="????? ??????????",
            section_summary="???????? ??????? ???????????? ? ??? ????????.",
            expected_topics=["??????? ????????????", "???????? ????????????"],
        ...
        prompt_content = mock_llm.calls[0]["messages"][0].content
        assert "КОНТРАКТ РАЗДЕЛА" in prompt_content
